...
const AnalyzeCswapArbV1: React.FC = () => {
  ...
  const startAnalysis = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/bot/cswap_arbitrage', {job_id: job_id });
  ...
  */
  ...
  return (
    <div className="bg-gray-900 text-gray-200 p-4 min-h-400">
      <h2 className="text-xl font-bold mb-4">Catsky AI CatBot: CSWAP Arbitrage</h2>
        ...
        {!buttonClicked && (
          ...
          >
            {isLoading ? (
              ...
              <span className="font-semibold">Find CSWAP Arbitrage</span>
            )}
          </button>
        )}
      </div>
...
export default AnalyzeCswapArbV1;